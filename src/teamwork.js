...
export function tickTeamwork(world,dt,crops){
 ...
 const child=s.npcs.find(n=>n.id==='child');
 if(child){child.timer=Math.max(0,(child.timer||0)-dt);const foe=s.wolves.find(w=>!w.dead&&!w.retreat&&distance(child,w)<75);if(foe&&!child.timer){world.damageEnemy(foe,child.role?.includes('Robot')?2:1);child.timer=3;child.intent='Defending home';}}
 const adults=s.npcs.filter(n=>n.id.startsWith('resident-')||n.id==='partner');
 for(const n of adults){
  n.workCooldown=Math.max(0,(n.workCooldown||0)-dt);n.combatCooldown=Math.max(0,(n.combatCooldown||0)-dt);
  const enemies=s.wolves.filter(e=>!e.dead&&!e.retreat&&distance(n,e)<340).sort((a,b)=>(targets.get(a)||0)-(targets.get(b)||0)||distance(n,a)-distance(n,b));
  const foe=enemies[0];
  if(foe){targets.set(foe,(targets.get(foe)||0)+1);n.intent='Defending the settlement';world.moveTo(n,foe,85,dt);
   if(distance(n,foe)<75&&!n.combatCooldown){world.damageEnemy(foe,n.role?.includes('Robot')?2:1.5);n.combatCooldown=2;s.effects.push({kind:'npc-action',action:'attack',x:n.x,y:n.y-30,tx:foe.x,ty:foe.y-20,until:s.time+.5});}continue;}
  const jobs=[];
  ...
  const available=jobs.filter(j=>!claimed.has(j.key)).sort((a,b)=>a.priority-b.priority||distance(n,a.target)-distance(n,b.target));
  ...
  claimed.add(job.key);n.assignment=job.key;n.intent={care:'Feeding and watering animals',harvest:'Harvesting crops',water:'Watering crops',plant:'Planting owned seeds'}[job.kind];world.moveTo(n,job.target,70,dt);
  ...
  s.effects.push({kind:'npc-action',action:job.kind,x:n.x,y:n.y-30,tx:t.x,ty:t.y,until:s.time+1});n.workCooldown=3;n.assignment=null;
 }
}
