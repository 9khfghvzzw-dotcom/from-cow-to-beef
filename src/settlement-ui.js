...
export function setupSettlement(world,{save,onBuild}){
  ...
  let category='crops';
  function refresh(){
    const s=world.state,f=s.family,ready=f.bond>=100||(s.npcs.find(n=>n.id==="partner")?.connection||0)>=80;
    ...
    if(category==='buildings')html+='<h3>A place to call home</h3><div class="shop-grid">'+Object.entries(BUILDINGS).map(([id,b])=>card(id,b.name,b.description||(id==='house'?'A small home. Adds 3 residents and a helpful neighbor who defends the settlement.':id==='barn'?'Shelter for your herd. Cows recover faster nearby.':'A defensive barrier. Enemies must break through it.'),button('build',id,`Place · ${b.price} gold`))).join('')+card('land',FARM_UPGRADES.land.name,`${s.landExpansions} extra fields unlocked · No limit. Each deed adds 6 plots; new fields extend south.`,button('farm-upgrade','land',`Buy field · ${FARM_UPGRADES.land.price} gold`))+'</div>';
    ...
    if(category==='household'){
      ...
      if(!f.partner)html+=Object.entries(COMPANIONS).map(([id,a])=>card(id,a.name,`${a.kind==='robot'?'Android':'Human'} · ${a.gender==='female'?'Woman':'Man'} · adult companion${id==='androidFemale'?' · blonde hair':''}<br>Requires a cottage.`,button('partner',id,a.price?`Build ${a.name} · ${a.price} gold`:`Invite ${a.name} · free`))).join('');
      else html+=card(f.companionDesign||f.partner,COMPANIONS[f.companionDesign||f.partner]?.name||'Companion',`Your companion · bond ${f.bond}/100`,'')+card('meal','Shared meal','Enjoy time together. Meals increase bond and have a 30-second interval.',button('bond','meal','Share a meal · 2 feed'))+card('family','Family room',f.child?`Your ${f.child==='hybrid'?'human–robot':f.child==='robot'?'robot android':'human'} child is home and helps defend the settlement.`:f.arrival!==null?`A new family member arrives in ${Math.ceil(f.arrival)}s.`:'Reach level 3 and bond 100 or connection 80 to begin a new family chapter.',f.partner==='robot'?button('family','hybrid','Human + android child · 80 gold',!ready||world.level<3||!!f.child||f.arrival!==null)+button('family','robot','Android + android child · 80 gold',!ready||world.level<3||!!f.child||f.arrival!==null):button('family','human','Start a family · 80 gold',!ready||world.level<3||!!f.child||f.arrival!==null));
      html+='<h3>Invite autonomous residents</h3><p>Residents work, learn and defend automatically. Extra cottages allow close residents to form families.</p>'+Object.entries(COMPANIONS).map(([id,a])=>card(id,a.name,'Independent adult resident',button('recruit',id,'Invite resident · '+(a.price||75)+' gold'))).join('');
      if(f.partner)html+='<h3>Companion appearance</h3><p>Choose a look for your current companion. Bond and family progress stay with you.</p>'+Object.entries(COMPANIONS).filter(([,a])=>a.kind===f.partner).map(([id,a])=>card(id,a.name,`${a.gender==='female'?'Woman':'Man'} · ${a.kind==='robot'?'Android':'Human'}`,button('companion-style',id,'Choose appearance · free'))).join('');
      html+='</div>';
    }
  ...
  content.onclick=e=>{
    ...
    if(b.dataset.companionStyle)world.styleCompanion(b.dataset.companionStyle);
    if(b.dataset.recruit)world.recruitCitizen(b.dataset.recruit);
