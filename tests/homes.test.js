...
test('entry requires proximity, all furniture charges once, player uses rooms and exits safely',()=>{
 ...
 assert.equal(s.coins,800);tickHomes(w,.1);s.player.needs.energy=20;assert.ok(useFurniture(w,'bed'));tickHomes(w,8);assert.equal(s.player.needs.energy,100);assert.equal(s.player.homeTask,null);
...
test('residents reserve different furniture, recover needs and return to farming',()=>{
 ...
 tickHomes(w,8);assert.equal(a.insideHome,null);assert.equal(b.insideHome,null);assert.equal(a.needs.energy,100);assert.equal(b.needs.hygiene,100);tickTeamwork(w,.1,CROPS);assert.ok(!a.homeTask);
});
